...
def load_csv_as_dataset(file_path: str):
    # Load dataset dari CSV
    df = pd.read_csv(file_path)
    
    # Mengubah DataFrame ke format yang diperlukan oleh to_gluonts_univariate
    dataset = []
    for index, row in df.iterrows():
        dataset.append({
            "timestamp": pd.date_range(start=row["timestamp"], periods=len(row) - 1, freq='D').tolist(),
            "values": row[1:].values
        })
...
def load_and_split_dataset(backtest_config: dict):
    ...
    ds = load_csv_as_dataset(hf_repo)
