...
class ChronosConfig:
    ...
    def create_tokenizer(self) -> "ChronosTokenizer":
        class_ = getattr(chronos_local, self.tokenizer_class)
...
class MeanScaleUniformBins(ChronosTokenizer):
    ...
    def _append_eos_token(
        ...
        eos_tokens = torch.full((batch_size, 1), fill_value=self.config.eos_token_id, device = "cuda:0")
        token_ids = torch.concat((token_ids, eos_tokens), dim=1)
        eos_mask = torch.full((batch_size, 1), fill_value=True, device="cuda:0")
    ...
    def output_transform(
        ...
        scale_unsqueezed = scale.unsqueeze(-1).unsqueeze(-1).to("cuda:0")
        indices = torch.clamp(
...
class ChronosPipeline:
    ...
    def predict(
        ...
        while remaining > 0:
            ...
            context_tensor = context_tensor.to(device='cuda')  # Move to GPU
